...
with open('NBAstats.csv', 'rb') as csv_file:
	...
	#used later with the external KMeans module
	...
	def mykmeans(data, clusters):
		...
		while changed == True:
			...
			for i in xrange(len(clusterSet)):
				for j in xrange(len(newAvgs[i])):
					clusterSet[i][j] = newAvgs[i][j]
	...
	#Number of instances in clusters 1, 2, and 3, respectively
	print(len(names[0]))
	print(len(names[1]))
	print(len(names[2]))
